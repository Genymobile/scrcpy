...
adb is deliberately **not bundled**. At runtime, `scrcpy-auto` uses the
system `adb` from `PATH` (or the executable selected through the `ADB`
environment variable), avoiding client/server version conflicts with an
already-running adb daemon.
...
- `--server prebuilt|none|/abs/path` — server source. `prebuilt` (default)
  fetches the pinned upstream v4.1 server (the fork's `server/src` is unmodified
...
- Install Android platform tools separately before connecting a device (for
  example, `sudo apt install adb` on Ubuntu).
