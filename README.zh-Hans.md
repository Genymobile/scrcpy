...
从源代码编译: [编译源码][BUILD] ([简化编译][BUILD_simple])
...
例如，可以镜像only one eye of the Oculus Go。
...
在这种模式下，原始按键事件 (扫描码) 被发送给设备，而与宿主机按键映射无关。因此，若键盘布局不匹配，需要在 Android 设备上进行配置，具体为 设置 → 系统 → 语言和输入法 → [键盘]。
