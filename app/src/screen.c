...
sc_screen_on_resize(struct sc_screen *screen, const SDL_WindowEvent *event) {
    ...
    if (screen->window_shown) {
        ...
        if (screen->flex_display) {
            ...
            sc_tick last_req = screen->resize_tracker.time;
            if (last_req
                    && screen->resize_tracker.size.width == width
                    && screen->resize_tracker.size.height == height
                    && sc_tick_now() < last_req + SC_TICK_FROM_MS(1000)) {
                // This resize event is the result of a previous resize request
                // triggered by a change in the frame's dimensions.
...
sc_screen_resize_to_fit(struct sc_screen *screen) {
    ...
    if (screen->render_fit == SC_RENDER_FIT_DISABLED) {
        set_aspect_ratio(screen, screen->content_size);
        sc_sdl_set_window_size(screen->window, screen->content_size);
        return;
    }
